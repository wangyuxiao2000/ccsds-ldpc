%*************************************************************%
% function: LDPC 对数域BP解码核心算法
% Author  : WangYuxiao
% Email   : wyxee2000@163.com
% Data    : 2024.1.26
% Version : V 1.0
%*************************************************************%
function [result_message, result_full, right_flag] = ldpc_llr_bp_decoder_core(yi, n, k, H, block_num, sigma2, iteratio_max)
   ...
   for block_cnt = 1:block_num
        ...
        if check ~= 0
            ...
            L_i = 2*yi_reg/sigma2;                                % 计算各比特节点的初始信息
        ...
        while check~=0 && num<iteratio_max
            ...
            for cnt = 1:size(check2bit, 2)
                ...
                if any(a(indices) == 0) % 防止dummy_bit在fai函数运算时产生NaN值
                    check_node(cnt) = 0;
                else
                    check_node(cnt) = prod(a(indices)) * fai(sum(fai(b(indices)))); 
                end
            end
...
function result = fai(x)
    result = log((exp(x) + 1) ./ (exp(x) - 1));
end